...
* **文書ステータス**: Draft v0.5
* **作成日**: 2026-07-09
* **更新**: ビルド確認はLinux上の`mingw-w64`クロスコンパイルで完結することが分かったため、Windows実機での作業は「実機でしか確認できないこと」だけに絞った。SPIKE-01・SPIKE-02・SPIKE-09相当(デバイス変更観測+最小限の自動復帰)に加え、**SPIKE-11(Audio Endpoint Registry本体)も実装完了**。`todo!()`は全て実装済み。実機での実行検証は、方針どおり開発がまとまった段階でまとめて行う。
