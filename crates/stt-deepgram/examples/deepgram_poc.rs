//! Live connectivity spike for the Deepgram adapter. Connects to the real Deepgram
//! streaming API, feeds it a synthesized sine wave (no real speech is available in
//! this environment), and prints every `SttEvent` as it arrives.
//!
//! Since the audio is a pure tone rather than speech, Deepgram is not expected to
...
//!
//! ```sh
//! DEEPGRAM_API_KEY=xxx cargo run --example deepgram_poc -p stt-deepgram
//! ```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let api_key = match std::env::var("DEEPGRAM_API_KEY") {
        ...
        _ => {
            eprintln!("DEEPGRAM_API_KEY is not set; export a real Deepgram API key to run this spike");
            std::process::exit(1);
        }
    };
    ...
    let config = SttSessionConfig::new(SAMPLE_RATE_HZ)
        ...
        // Needed for Deepgram to emit SpeechStarted/UtteranceEnd at all, so this
        // spike can exercise the SttEvent::SpeechStarted/SpeechEnded path too.
