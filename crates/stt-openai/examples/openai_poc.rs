//! Live connectivity spike for the OpenAI Realtime transcription adapter. Connects
//! to the real OpenAI Realtime API, feeds it a synthesized sine wave (no real speech
//! is available in this environment), and prints every `SttEvent` as it arrives.
//!
//! Since the audio is a pure tone rather than speech, OpenAI is not expected to
//! return a meaningful transcript — this spike is only about confirming the
//! WebSocket handshake, `session.update` framing, and event parsing round-trip
//! against the live API.
//!
//! Requires a real OpenAI API key with Realtime API access — this crate has not been
//! verified against a live connection (see README.md "Status").
//!
//! Audio must be 24kHz mono (`stt_openai::OpenAiProvider` rejects any other rate;
//! this crate does not resample), so this spike generates the tone at 24kHz directly
//! rather than at the 16kHz used by `stt-deepgram`'s equivalent spike.
//!
//! Run with:
//!
//! ```sh
//! OPENAI_STT_API_KEY=xxx cargo run --example openai_poc -p stt-openai
//! ```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let api_key = match std::env::var("OPENAI_STT_API_KEY") {
        Ok(key) if !key.is_empty() => key,
        _ => {
            eprintln!(
                "OPENAI_STT_API_KEY is not set; export a real OpenAI API key with \
                 Realtime API access to run this spike"
            );
            std::process::exit(1);
        }
    };
    ...
    let config = SttSessionConfig::new(SAMPLE_RATE_HZ)
        ...
        // Needed for OpenAI to auto-segment audio into successive turns at all, so
        // this spike can exercise SttEvent::SpeechStarted/SpeechEnded and get more
        // than one FinalTranscript out of a multi-second stream.
